...
public class RopeTest extends TestCase {
	...
	public void testConcatenationIndexOf() {
		Rope left  = Rope.BUILDER.build("0000");
		Rope right = Rope.BUILDER.build("1111");
		ConcatenationRope r = new ConcatenationRope(left, right);
		assertEquals("indexOf string spanning left and right nodes of a concatenation rope", 2, r.indexOf("0011"));
	}
	...
	public void testTrim() {
		...
		Rope x2 = new FlatCharSequenceRope("\u0002 67	       \u0007");
		testTrim(x1, x2);
		x1 = new RopeBuilder().build(x1.toString());
		x2 = new RopeBuilder().build(x2.toString());
		testTrim(x1, x2);
		
	}

	private void testTrim(Rope x1, Rope x2) {
		Rope x3 = new ConcatenationRope(x1, x2);
	...
	public void testIndexOfBug() {
		...
		{   // extra test, bwatkins
			...
			Rope r1 = new FlatCharArrayRope(s1.toCharArray());
			Assert.assertEquals(s1.indexOf(s2), r1.indexOf(s2));
		}
	}
}
