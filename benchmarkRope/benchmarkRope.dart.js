...
function benchmarkRope() {
  this.testCount = (0);
  this.maxTestTime = (0);
  this.maxIteration = (0);
}
benchmarkRope.prototype.run = function() {
  var canvas = get$$document().query("#canvas");
  this.ctx = canvas.getContext("2d");
  this.results = new Array();
  var cb = new DefaultConcatOperation();
  this.ropeTest((100), cb);
  this.ropeTest((500), cb);
  this.ropeTest((700), cb);
  this.ropeTest((1000), cb);
  this.ropeTest((2000), cb);
  this.ropeTest((3000), cb);
  this.ropeTest((4000), cb);
  this.ropeTest((5000), cb);
  var cb2 = new DefaultConcatOperation();
  this.ropeTest((5000), cb2);
  var cb3 = new DefaultConcatOperation();
  this.ropeTest((5000), cb3);
  this.ropeTest((5000), cb3);
  this.ropeTest((5000), cb3);
  this.ropeTest((5000), cb3);
  var $$list = this.results;
  for (var $$i = $$list.iterator(); $$i.hasNext(); ) {
    var eachResult = $$i.next();
    this.drawCircle(eachResult.iter / this.maxIteration * (300) - (2), (300) - (eachResult.time / this.maxTestTime * (300)));
  }
}
benchmarkRope.prototype.ropeTest = function(iterations, operationFunction) {
  var timer = new StopwatchImplementation();
  var st = "a";
  timer.start();
  for (var i = (0);
   i < iterations; i++) {
    var num0 = "1";
    st = operationFunction.operate(num0);
  }
  timer.stop();
  var testTime = timer.elapsedInUs();
  this.write($add$(iterations.toString(), (" iterations took " + testTime + " us")));
  this.testCount++;
  var result = new TestResult();
  result.iter = iterations;
  result.time = testTime;
  this.results.add(result);
  if (testTime > this.maxTestTime) {
...
benchmarkRope.prototype.write = function(message) {
  var $0;
  ($0 = get$$document().query("#status")).set$innerHTML($add$($0.get$innerHTML(), ($add$("<br>", message))));
}
benchmarkRope.prototype.drawCircle = function(x, y) {
  this.ctx.beginPath();
...
function DefaultConcatOperation() {
  this.localSt = "";
}
DefaultConcatOperation.prototype.operate = function(inputSt) {
  this.localSt = this.localSt.concat(inputSt);
  return this.localSt;
}
function TestResult() {
